    ...
    <form method="POST" action="{{ route('comment.update', ['commentaire' => $comment->id]) }}"
        ...
        <div class="flex flex-col gap-4 h-full">
            <input type="text" class="bg-gray-600 h-10 w-full rounded px-3 text-white" name="title"
                   placeholder="Veuillez écrire le titre de votre commentaire" value="{{$comment->title}}">
            <textarea name="description" class="bg-blue-100 h-50 md:flex-grow p-3 rounded resize-none"
                      placeholder="Votre commentaire">{{$comment->description}}</textarea>
        </div>
        <input type="hidden" name="apprentis_id" value="{{ $comment->apprentis_id }}">
