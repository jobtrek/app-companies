...
@section('content')
    <div class="shadow-lg rounded-xl md:mt-40">
            ...
            <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-6">
                <form id="profilForm" action="{{ route('users.update', ['user' => $user->id]) }}" method="POST"
                    ...
                    <div class="flex gap-5">
                        <div class="bg-green-50 p-5 rounded-xl shadow-md flex items-center gap-5 w-full">
                        ...
                        <div class="relative inline-block text-left">
                            ...
                            <div class="absolute right-0 mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg z-10 hidden"
                                ...
                                <form action="{{ route('users.updateEntreprise', ['user' => $user->id]) }}" method="POST"
                                    ...
                                    <label for="entreprise_id" class="block text-sm font-semibold text-gray-700">Choisir
                                        une
                                        entreprise
                                        :</label>
                        ...
                        <div class="relative inline-block text-left">
                            ...
                            <div class="absolute right-0 mt-2 w-48 bg-white border border-gray-300 rounded-md shadow-lg z-10 hidden"
                                ...
                                <form action="{{ route('users.updateCoach', ['user' => $user->id]) }}" method="POST"
                                    ...
                                    <label for="coach_id" class="block text-sm font-semibold text-gray-700">Choisir un
                                        coach
                                        :</label>
        ...
        @can('admin')
            <form method="POST" action="{{ route('users.delete', $user->id) }}" class="flex justify-center mt-8 m-10">
                @csrf
                @method('DELETE')
                <button type="submit"
