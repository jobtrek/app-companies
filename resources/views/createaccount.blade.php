...
@section('content')
    <div class="flex items-center justify-center mt-20 md:mt-40">
        <div class="w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl relative rounded-lg">
            ...
            <div class="bg-gray-200 p-10 sm:p-8 pt-20 rounded-3xl flex flex-col gap-6 ">
                ...
                <form method="post" action="{{ route('user.create') }}" class="flex flex-col gap-6 mt-4 pt-6">
                    @csrf
                    <img src="{{ asset('images/createaccount_page/edit.svg') }}" alt="Icône éditer"
                        class="w-24 h-24 object-cover absolute -top-12 left-1/2 transform -translate-x-1/2" loading="lazy"
                        role="img" />
                    <div class="relative pl-12">
                        <img src="{{ asset('images/createaccount_page/account.svg') }}" alt="Icône utilisateur"
                            class="absolute left-0 top-1/2 transform -translate-y-1/2 w-15 h-15" loading="lazy"
                            role="img" />
                        <div class="grid grid-cols-2 ml-10 gap-4 justify-center w-full">
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="role[]" value="formateur_informaticien"
                                    class="form-checkbox text-blue-600">
                                <span class="ml-2">Formateur Informaticien</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="role[]" value="formateur_commerce"
                                    class="form-checkbox text-blue-600">
                                <span class="ml-2">Formateur Commerce</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="role[]" value="apprenti_informaticien"
                                    class="form-checkbox text-blue-600">
                                <span class="ml-2">Apprenti Informaticien</span>
                            </label>
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="role[]" value="apprenti_commerce"
                                    class="form-checkbox text-blue-600">
                    ...
                    <div class="relative">
                        <label class="inline-flex items-center w-full">
                            <select class="border p-2 pl-12 rounded-lg bg-gray-300 placeholder-gray-600 text-center w-full"
                                name="domain_id">
                                <option class="text-transparent text-center">Veuillez séléctionner le domain de
                                    l'utilisateur
                                </option>
    ...
    <a href="{{ route('domain.create') }}"
        class="flex items-center justify-center md:justify-start px-3 py-2 rounded-md gap-2 bg-green-100 border-2 hover:bg-green-50 absolute right-10">
        <span class="md:inline">&rarr;</span>
    </a>
